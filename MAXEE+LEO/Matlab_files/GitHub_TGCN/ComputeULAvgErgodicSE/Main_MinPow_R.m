...
%% parameter seting
% BS parameter
N_BS=128;                                                                  % number of Antenna at BS 
...
R=50:50:200;                                                              % distance of BS and user[meter]
nCell_radius = length(R);
...
PT_final1 = zeros(ch_reali,nCell_radius);
PT_final1_3 = zeros(ch_reali,nCell_radius);
PT_final1_4 = zeros(ch_reali,nCell_radius);
PT_final2 = zeros(ch_reali,nCell_radius);   
PT_final3 = zeros(ch_reali,nCell_radius);              
PT_final4 = zeros(ch_reali,nCell_radius);
PT_final5 = zeros(ch_reali,nCell_radius);              
PT_final6 = zeros(ch_reali,nCell_radius);
...
for ch=1:1:ch_reali 
    for r=1:1:length(R)
        ...
        for tu=1:1:Total_U
            d_2D(tu)=norm([x_location(tu) y_location(tu)],2);                     % unit is meter    
            [PL(tu),path_loss_dB(tu)]=PathLoss(fc,h_BS,d_2D(tu));                            % path loss model
            [CH] =Satellite_channel(N_per_MS,N_BS,N_sub,1,Nray,ang_spread,path_loss_dB(tu),fc);
            H_user(:,:,:,tu)=CH(:,:,:,1)/sqrt(PL(tu));
        end
...
Sel_User_ind = selected_subcarriers(1,1:U);
    ...
    for u=1:1:U
        ...
        % Compute hybrid beamforming matrix for TU-BD-SVD scheme
    ...
    TU_result_CH_3;
        ...
        sel_sub_index = N_sub/2;
        all_parallel_chgain = zeros(N_per_MS,U);
        for sub_index=1:1:N_sub
            for u=1:1:U
            ...
            for u=1:1:U
                ...
                if sub_index == sel_sub_index 
                    all_parallel_chgain(:,u) = svd(FD_U_tilte(:,rank_int_ch(u,sub_index)+1:end,u,sub_index)'*Sel_H_user(:,:,sub_index,u)).';
                end
            end     
        end
    ...
    TU_result_CH_4;
        ...
        sel_sub_index_2 = N_sub/2;
        all_parallel_chgain_2 = zeros(N_per_MS,U);
    for sub_index=1:1:N_sub
        for u=1:1:U
        ...
        for u=1:1:U
            ...
            if sub_index == sel_sub_index_2 
                all_parallel_chgain_2(:,u) = svd(FD_U_tilte_2(:,rank_int_ch_2(u,sub_index)+1:end,u,sub_index)'*Sel_H_user_2(:,:,sub_index,u)).';
            end
        end     
    end
...
norm_power1 = 10*log10(PT_final_mean1/PT_final_mean6(nCell_radius));
norm_power1_3 = 10*log10(PT_final_mean1_3/PT_final_mean6(nCell_radius));
norm_power1_4 = 10*log10(PT_final_mean1_4/PT_final_mean6(nCell_radius));
norm_power2 = 10*log10(PT_final_mean2/PT_final_mean6(nCell_radius));
norm_power3 = 10*log10(PT_final_mean3/PT_final_mean6(nCell_radius));
norm_power4 = 10*log10(PT_final_mean4/PT_final_mean6(nCell_radius));
norm_power5 = 10*log10(PT_final_mean5/PT_final_mean6(nCell_radius));
norm_power6 = 10*log10(PT_final_mean6/PT_final_mean6(nCell_radius));

figure
plot(R,norm_power1,'bo-','LineWidth',1.2,'DisplayName','Proposed (US)')
hold on
plot(R,norm_power1_3,'mo-.','LineWidth',1.2,'DisplayName','Proposed (Reduce Complexity,K/4)(US)')
plot(R,norm_power2,'-*','color',[0.1500 0.8000 0.3000],'LineWidth',1.2,'DisplayName','Fully-Digital (US)')
plot(R,norm_power5,'-^','color',[0.8500 0.3250 0.0980],'LineWidth',1.2,'DisplayName','Modified\_ACMD [35] (US)')

plot(R,norm_power3,'rs-','LineWidth',1.2,'DisplayName','Proposed (RUS)')
plot(R,norm_power1_4,'bs-.','LineWidth',1.2,'DisplayName','Proposed (Reduce Complexity,K/4)(RUS)')
plot(R,norm_power4,'-.*','color',[0.1500 0.8000 0.3000],'LineWidth',1.2,'DisplayName','Fully-Digital (RUS)')
plot(R,norm_power6,'-.^','color',[0.8500 0.3250 0.0980],'LineWidth',1.2,'DisplayName','Modified\_ACMD [35] (RUS)')
