...
Total_U = 20;
U=6;                                                                       % number of users
Ns_per_MS=2;                                                               % number of data streams per user(equal data stream allocation)
...
N_bits = 75;
System_BW=(20*10^6);                                                       % System Bandwidth(20 MHz)
...
Nray=10;
antenna_array = 0;
PT_final1 = zeros(ch_reali,nNbs);
PT_final1_3 = zeros(ch_reali,nNbs);
PT_final1_4 = zeros(ch_reali,nNbs);
PT_final2 = zeros(ch_reali,nNbs);  
PT_final3 = zeros(ch_reali,nNbs);       
PT_final4 = zeros(ch_reali,nNbs);
PT_final5 = zeros(ch_reali,nNbs);       
PT_final6 = zeros(ch_reali,nNbs);
...
Sel_User_ind = selected_subcarriers(1,1:U);
    ...
    Sel_H_user = zeros(N_BS(r),N_per_MS,N_sub,U);
    for u=1:1:U
        Sel_H_user(:,:,:,u) = H_user(:,:,:,Sel_User_ind(u));     
    end
    %% TUMD
        % analog precoder at MS via Tensor unfolding
        long_unfold_Hu = zeros((N_sub)*N_BS(r),N_per_MS,U); 
        long_unfold_F_RFu = zeros(N_per_MS,Nrf_per_MS,U);
        for u=1:1:U
            for sub_index=1:1:N_sub
                long_unfold_Hu((sub_index-1)*N_BS(r)+1:1:sub_index*N_BS(r),:,u) = Sel_H_user(:,:,sub_index,u);
            end
        ...
        horiz_unfold_Hu = zeros(N_BS(r),Nrf_per_MS*(N_sub),U);
        horiz_unfold_W_RF = [];
        eigval_buff = zeros(N_BS(r),U);
        eigvec_buff = zeros(N_BS(r),N_BS(r),U);
        ...
        for u=1:1:U
            horiz_unfold_W_RF = [horiz_unfold_W_RF,extract_phase_v2(eigvec_buff(:,1:1:Ns_TU_alloc(u),u),N_BS(r))];
        end   
        ...
        TU_W_HBFu = zeros(N_BS(r),max(Ns_TU_alloc),U,N_sub);
        for sub_index=1:1:N_sub       
            for u=1:1:U
    ...
    TU_H_eff = zeros(N_BS(r),Ns_total,N_sub);
    TU_W_HBF_fixed = zeros(N_BS(r),Ns_total,N_sub);
    ...
    for sub_index=1:1:N_sub       
        for u=1:1:U
            ...
            Noise_eff(:,sub_index) = N0*norm(TU_W_HBF_fixed(:,:,sub_index)')^2;
            Noise_eff;
            TU_result_CH(:,sub_index) = norm(TU_W_HBF_fixed(:,:,sub_index)'*TU_H_eff(:,:,sub_index))^2;  %% Heff
            end
        end  
    end
    TU_result_CH;
        ...
        %% TUMD(K/4)
    % analog precoder at MS via Tensor unfolding
    long_unfold_Hu_3 = zeros((N_sub/4)*N_BS(r),N_per_MS,U); 
    long_unfold_F_RFu_3 = zeros(N_per_MS,Nrf_per_MS,U);
    for u=1:1:U
        for sub_index2=2:4:14
            long_unfold_Hu_3((sub_index2-1)*N_BS(r)+1:1:sub_index2*N_BS(r),:,u) = Sel_H_user(:,:,sub_index2,u);
        end
    ...
    % analog combiner at BS via Tensor unfolding and RF chain assignment
    horiz_unfold_Hu_3 = zeros(N_BS(r),Nrf_per_MS*(N_sub/4),U);
    horiz_unfold_W_RF_3 = [];
    eigval_buff_3 = zeros(N_BS(r),U);
    eigvec_buff_3 = zeros(N_BS(r),N_BS(r),U);
    ...
    for u=1:1:U
        horiz_unfold_W_RF_3 = [horiz_unfold_W_RF_3,extract_phase_v2(eigvec_buff_3(:,1:1:Ns_TU_alloc_3(u),u),N_BS(r))];
    end
    ...
    TU_W_HBFu_3 = zeros(N_BS(r),max(Ns_TU_alloc_3),U,N_sub);
    for sub_index=1:1:N_sub       
        for u=1:1:U
    ...
    %% Effective channel calculation
    TU_H_eff_3 = zeros(N_BS(r),Ns_total,N_sub);
    TU_W_HBF_fixed_3 = zeros(N_BS(r),Ns_total,N_sub);
    ...
    for sub_index=1:1:N_sub       
        for u=1:1:U
            ...
            Noise_eff_3(:,sub_index) = N0*norm(TU_W_HBF_fixed_3(:,:,sub_index)')^2;
            Noise_eff_3;
            TU_result_CH_3(:,sub_index) = norm(TU_W_HBF_fixed_3(:,:,sub_index)'*TU_H_eff_3(:,:,sub_index))^2;  %% Heff
            end  
        end
    end
    TU_result_CH_3;
       ...
       %% Fully-digital beamforming
        FD_Hu = zeros(N_BS(r),N_per_MS*(U),N_sub); 
        FD_U_tilte = zeros(N_BS(r),N_BS(r),U,N_sub);
        ...
        FD_W_u = zeros(N_BS(r),max(Ns_FD_alloc),U,N_sub);
        ...
        %% Effective channel calculation
    FD_H_eff = zeros(N_BS(r),Ns_total,N_sub);
    FD_W_HBF_fixed = zeros(N_BS(r),Ns_total,N_sub);
    ...
    for sub_index=1:1:N_sub       
        for u=1:1:U
            ...
            FD_Noise_eff(:,sub_index) = N0*norm(FD_W_HBF_fixed(:,:,sub_index)')^2;
            FD_result_CH(:,sub_index) = norm(FD_W_HBF_fixed(:,:,sub_index)'*FD_H_eff(:,:,sub_index))^2;  %% Heff
            end
        end
    end
    FD_result_CH;
        ...
        for nu = 1:U
            ...
            while bits > 0
                ...
                MODCOD_Index_table2(st+x(1)-1,y(1)) = MODCOD_Index_table2(st+x(1)-1,y(1)) + 1;                             % Update MODCOD index table
        ...
        chavg_L_sinvec_buff = zeros(N_BS(r),N_per_MS,U);
        % analog precoder
        for u=1:1:U
            Q = 0;
            for sub_index=1:1:N_sub
        ...
        for u=1:1:U
            chavg_W_RF = [chavg_W_RF,extract_phase_v2(chavg_L_sinvec_buff(:,1:1:Ns_CHAVG_alloc(u),u),N_BS(r))];
        end
        ...
        chavg_W_HBFu = zeros(N_BS(r),max(Ns_CHAVG_alloc),U,N_sub);
        for sub_index=1:1:N_sub       
            for u=1:1:U
        ...
        %% Effective channel calculation
    chavg_H_eff = zeros(N_BS(r),Ns_total,N_sub);
    chavg_W_HBF_fixed = zeros(N_BS(r),Ns_total,N_sub);
    ...
    for sub_index=1:1:N_sub       
        for u=1:1:U
            ...
            chavg_Noise_eff(:,sub_index) = N0*norm(chavg_W_HBF_fixed(:,:,sub_index)')^2;
            chavg_result_CH(:,sub_index) = norm(chavg_W_HBF_fixed(:,:,sub_index)'*chavg_H_eff(:,:,sub_index))^2;  %% Heff
            end
        end
    end
    chavg_result_CH;
        ...
        Sel_User_ind_2 = randperm(Total_U,U);
       %% 這邊是為了把被選到的使用者通道挑出來存放
    Sel_H_user_2 = zeros(N_BS(r),N_per_MS,N_sub,U);
    for u=1:1:U
        Sel_H_user_2(:,:,:,u) = H_user(:,:,:,Sel_User_ind_2(u));     
    end
    %% TUMD
        % analog precoder at MS via Tensor unfolding
        long_unfold_Hu_2 = zeros((N_sub)*N_BS(r),N_per_MS,U); 
        long_unfold_F_RFu_2 = zeros(N_per_MS,Nrf_per_MS,U);
        for u=1:1:U
            for sub_index=1:1:N_sub
                long_unfold_Hu_2((sub_index-1)*N_BS(r)+1:1:sub_index*N_BS(r),:,u) = Sel_H_user_2(:,:,sub_index,u);
            end
        ...
        horiz_unfold_Hu_2 = zeros(N_BS(r),Nrf_per_MS*(N_sub),U);
        eigval_buff_2 = zeros(N_BS(r),U);
        eigvec_buff_2 = zeros(N_BS(r),N_BS(r),U);
    for u=1:1:U
        for sub_index=1:1:N_sub
    ...
    h_unf_W_RF_fixed_2 = [];
    for u=1:1:U
        h_unf_W_RF_fixed_2 = [h_unf_W_RF_fixed_2,extract_phase_v2(eigvec_buff_2(:,1:1:Ns_equal_alloc_2(u),u),N_BS(r))];
    end
    % BD(ignore the noise)+SVD beamforming based on effective channel
    ...
    TU_W_HBFu_fixed_2 = zeros(N_BS(r),max(Ns_equal_alloc_2),U,N_sub);
    for sub_index=1:1:N_sub       
        for u=1:1:U
        ...
        %% Effective channel calculation
    TU_H_eff_2 = zeros(N_BS(r),Ns_total,N_sub);
    TU_W_HBF_fixed_2 = zeros(N_BS(r),Ns_total,N_sub);
    ...
    for sub_index=1:1:N_sub       
        for u=1:1:U
            ...
            Noise_eff_2(:,sub_index) = N0*norm(TU_W_HBF_fixed_2(:,:,sub_index)')^2;
            Noise_eff_2;
            TU_result_CH_2(:,sub_index) = norm(TU_W_HBF_fixed_2(:,:,sub_index)'*TU_H_eff_2(:,:,sub_index))^2;  %% Heff
            end
        end  
    end
    TU_result_CH_2;
        ...
        MODCOD_Index_table3;
        bits_table3 = data_rate_st(MODCOD_Index_table3+1);
        %% Calculate transmitted power
        for k = 1:N_sub
            for nf3 = 1:Ns_total
        ...
        %% TUMD(K/4)
    % analog precoder at MS via Tensor unfolding
    long_unfold_Hu_4 = zeros((N_sub/4)*N_BS(r),N_per_MS,U); 
    long_unfold_F_RFu_4 = zeros(N_per_MS,Nrf_per_MS,U);
    for u=1:1:U
        for sub_index3=2:4:14
            long_unfold_Hu_4((sub_index3-1)*N_BS(r)+1:1:sub_index3*N_BS(r),:,u) = Sel_H_user_2(:,:,sub_index3,u);
        end
    ...
    % analog combiner at BS via Tensor unfolding and RF chain assignment
    horiz_unfold_Hu_4 = zeros(N_BS(r),Nrf_per_MS*(N_sub/4),U);
    horiz_unfold_W_RF_4 = [];
    eigval_buff_4 = zeros(N_BS(r),U);
    eigvec_buff_4 = zeros(N_BS(r),N_BS(r),U);
    ...
    for u=1:1:U
        horiz_unfold_W_RF_4 = [horiz_unfold_W_RF_4,extract_phase_v2(eigvec_buff_4(:,1:1:Ns_TU_alloc_4(u),u),N_BS(r))];
    end
    ...
    TU_W_HBFu_4 = zeros(N_BS(r),max(Ns_TU_alloc_4),U,N_sub);
    for sub_index=1:1:N_sub       
        for u=1:1:U
    ...
    %% Effective channel calculation
    TU_H_eff_4 = zeros(N_BS(r),Ns_total,N_sub);
    TU_W_HBF_fixed_4 = zeros(N_BS(r),Ns_total,N_sub);
    ...
    for sub_index=1:1:N_sub       
        for u=1:1:U
            ...
            Noise_eff_4(:,sub_index) = N0*norm(TU_W_HBF_fixed_4(:,:,sub_index)')^2;
            Noise_eff_4;
            TU_result_CH_4(:,sub_index) = norm(TU_W_HBF_fixed_4(:,:,sub_index)'*TU_H_eff_4(:,:,sub_index))^2;  %% Heff
            end  
        end
    end
    TU_result_CH_4;
        ...
        PT_final_table1_4;
        %% Fully-digital beamforming
        FD_Hu_2 = zeros(N_BS(r),N_per_MS*(U),N_sub); 
        FD_U_tilte_2 = zeros(N_BS(r),N_BS(r),U,N_sub);
    ...
    for sub_index=1:1:N_sub
        ...
        FD_W_u_2 = zeros(N_BS(r),max(Ns_FD_alloc_2),U,N_sub);
    ...
    for sub_index=1:1:N_sub
        ...
        %% Effective channel calculation
    FD_H_eff_2 = zeros(N_BS(r),Ns_total,N_sub);
    FD_W_HBF_fixed_2 = zeros(N_BS(r),Ns_total,N_sub);
    ...
    for sub_index=1:1:N_sub       
        for u=1:1:U
            ...
            FD_Noise_eff_2(:,sub_index) = N0*norm(FD_W_HBF_fixed_2(:,:,sub_index)')^2;
            FD_result_CH_2(:,sub_index) = norm(FD_W_HBF_fixed_2(:,:,sub_index)'*FD_H_eff_2(:,:,sub_index))^2;  %% Heff
            end
        end
    end
    FD_result_CH_2;
        ...
        Unit_bits = data_rate_change(1)*ones(Ns_total,N_sub);
        SINR = switching_level_change(1)*ones(Ns_total,N_sub); 
        MODCOD_Index_table4 = zeros(Ns_total,N_sub);                                                         % Set up MCS index table
        ...
        MODCOD_Index_table4;
        bits_table4 = data_rate_st(MODCOD_Index_table4+1);
        %% Calculate transmitted power
        for k = 1:N_sub
            for nf4 = 1:Ns_total
        ...
        chavg_L_sinvec_buff_2 = zeros(N_BS(r),N_per_MS,U);
        % analog precoder
        for u=1:1:U
            Q_2 = 0;
            for sub_index=1:1:N_sub
        ...
        for u=1:1:U
            chavg_W_RF_2 = [chavg_W_RF_2,extract_phase_v2(chavg_L_sinvec_buff_2(:,1:1:Ns_CHAVG_alloc_2(u),u),N_BS(r))];
        end
        ...
        chavg_W_HBFu_2 = zeros(N_BS(r),max(Ns_CHAVG_alloc_2),U,N_sub);
        for sub_index=1:1:N_sub       
            for u=1:1:U
    ...
    %% Effective channel calculation
    chavg_H_eff_2 = zeros(N_BS(r),Ns_total,N_sub);
    chavg_W_HBF_fixed_2 = zeros(N_BS(r),Ns_total,N_sub);
    ...
    for sub_index=1:1:N_sub       
        for u=1:1:U
            ...
            chavg_Noise_eff_2(:,sub_index) = N0*norm(chavg_W_HBF_fixed_2(:,:,sub_index)')^2;
            chavg_result_CH_2(:,sub_index) = norm(chavg_W_HBF_fixed_2(:,:,sub_index)'*chavg_H_eff_2(:,:,sub_index))^2;  %% Heff
            end
        end
    end
    chavg_result_CH_2;
        ...
        Unit_bits = data_rate_change(1)*ones(Ns_total,N_sub);
        SINR = switching_level_change(1)*ones(Ns_total,N_sub); 
        MODCOD_Index_table6 = zeros(Ns_total,N_sub);                                                         % Set up MCS index table
...
norm_power1 = 10*log10(PT_final_mean1/PT_final_mean4(1));
norm_power1_3 = 10*log10(PT_final_mean1_3/PT_final_mean4(1));
norm_power1_4 = 10*log10(PT_final_mean1_4/PT_final_mean4(1));
norm_power2 = 10*log10(PT_final_mean2/PT_final_mean4(1));
norm_power3 = 10*log10(PT_final_mean3/PT_final_mean4(1));
norm_power4 = 10*log10(PT_final_mean4/PT_final_mean4(1));
norm_power5 = 10*log10(PT_final_mean5/PT_final_mean4(1));
norm_power6 = 10*log10(PT_final_mean6/PT_final_mean4(1));

figure
plot(N_BS,norm_power1,'bo-','LineWidth',1.2,'DisplayName','Proposed (US)')
hold on
plot(N_BS,norm_power1_3,'mo-.','LineWidth',1.2,'DisplayName','Proposed (Reduce Complexity,K/4)(US)')
plot(N_BS,norm_power2,'-*','color',[0.1500 0.8000 0.3000],'LineWidth',1.2,'DisplayName','Fully-Digital (US)')
plot(N_BS,norm_power5,'-^','color',[0.8500 0.3250 0.0980],'LineWidth',1.2,'DisplayName','Modified\_ACMD [35] (US)')

plot(N_BS,norm_power3,'rs-','LineWidth',1.2,'DisplayName','Proposed (RUS)')
plot(N_BS,norm_power1_4,'bs-.','LineWidth',1.2,'DisplayName','Proposed (Reduce Complexity,K/4)(RUS)')
plot(N_BS,norm_power4,'-.*','color',[0.1500 0.8000 0.3000],'LineWidth',1.2,'DisplayName','Fully-Digital (RUS)')
plot(N_BS,norm_power6,'-.^','color',[0.8500 0.3250 0.0980],'LineWidth',1.2,'DisplayName','Modified\_ACMD [35] (RUS)')

legend('Location','best')
xlabel('Number of BS antennas (N_B_S)')
